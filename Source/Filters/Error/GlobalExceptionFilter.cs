...
public class GlobalExceptionFilter(ILogger<GlobalExceptionFilter> logger) : IExceptionFilter
{
  public void OnException(ExceptionContext context)
  {
    ...
    else if (context.Exception is JsonException)
    {
      ...
      result = new ObjectResult(new
      {
        message = ErrorConstants.ModelValidationError,
        errors = validationErrors
      });
    }
    else if (context.HttpContext.Items.ContainsKey("FluentValidationErrors"))
    {
      var fluentResult = (IDictionary<string, string[]>)context.HttpContext.Items["FluentValidationErrors"]!;
      result = new ObjectResult(new
      {
        message = ErrorConstants.ModelValidationError,
        errors = fluentResult
    ...
    else if (context.Exception is JsonException)
    {
      ...
      result = new ObjectResult(new
      {
        message = context.HttpContext.Response.Body,
        errors = validationErrors
      })
      {
